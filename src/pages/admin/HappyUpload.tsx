...
const HappyUpload = () => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (file.type.startsWith('audio/')) {
                ...
                toast.error('Please select a valid audio file (MP3).');
            }
        }
    };
    ...
    const handleUpload = async (e: React.FormEvent) => {
        ...
        if (!audioFile || !title || !description) {
            toast.error('Please fill in all fields and select an audio track.');
            return;
        }
        ...
        try {
            ...
            // 2. Transmit to Cloudinary (auto-detects audio)
            ...
            await addDoc(collection(db, "happy_tracks"), {
                title,
                description,
                audioUrl,
            ...
            toast.success('Happy track synchronized successfully');
            navigate('/manager/nexus');
        } catch (error: any) {
            console.error('Upload error:', error);
            toast.error(error.message || 'Failed to initialize audio sync');
        } finally {
            setLoading(false);
        }
    };
            ...
            >
                ...
                <div className="mb-12">
                    <div className="flex items-center gap-4 mb-3">
                        ...
                        <h1 className="text-3xl font-display font-light tracking-tight">Audio <span className="text-orange-400 italic">Lab</span></h1>
                    </div>
                    <p className="text-white/30 text-sm font-light leading-relaxed">
                        Synchronize a new euphoric track to the Happy Core. This asset will be indexed for all radiant citizens.
                    </p>
                </div>
                ...
                <form onSubmit={handleUpload} className="space-y-8">
                    <div className="space-y-6">
                        <div className="relative group/input">
                            <label className="text-[10px] text-white/20 uppercase tracking-[0.2em] ml-1 mb-2 block font-bold">Track Title</label>
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    placeholder="Euphoric Resonance..."
                        ...
                        <div className="relative group/input">
                            <label className="text-[10px] text-white/20 uppercase tracking-[0.2em] ml-1 mb-2 block font-bold">Track Narrative</label>
                            <div className="relative">
                                ...
                                <textarea
                                    placeholder="Describe the radiant energy..."
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] text-white/20 uppercase tracking-[0.2em] ml-1 mb-2 block font-bold">Audio File (MP3)</label>
                            <div
                                ...
                                className={cn(
                                    ...
                                    audioFile ? "border-orange-400/30 bg-orange-400/5" : "border-white/5 bg-white/[0.02] hover:bg-white/[0.05] hover:border-orange-400/20"
                                )}
                            >
                                <input
                                    type="file"
                                ...
                                {audioFile ? (
                                    <div className="flex flex-col items-center gap-2">
                                        ...
                                        <span className="text-xs font-medium text-white/60">{audioFile.name}</span>
                                        <span className="text-[10px] text-white/20 uppercase tracking-widest">Aura Scan Ready</span>
                                    </div>
                                ) : (
                                    <div className="flex flex-col items-center gap-3">
                                        <Headphones className="w-8 h-8 text-white/10 group-hover:text-orange-400 transition-all" />
                                        <p className="text-xs text-white/30">Drag track or click to browse</p>
                                        <p className="text-[8px] text-white/10 uppercase tracking-[0.3em] font-bold">MP3 (MAX // 50MB)</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    >
                        {loading ? (
                            <>
                                ...
                                <span>Synchronizing Aura...</span>
                            </>
                        ) : (
                            <>
                                <Sparkles className="w-5 h-5" />
                                <span>Initialize Audio Sync</span>
                            </>
                        )}
                    </Button>
                </form>
